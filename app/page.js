...
function isYesterday(dateString) {
  ...
  return reportDate.getFullYear() === yesterday.getFullYear() &&
         reportDate.getMonth() === yesterday.getMonth() &&
         reportDate.getDate() === yesterday.getDate()
}

// Función para filtrar reportes del día anterior
function filterYesterdayReports(reportes) {
  return reportes.filter(reporte => isYesterday(reporte.fecha_reporte))
}

export default function Home() {
  const [reportes, setReportes] = useState([])
  const [reportesYesterday, setReportesYesterday] = useState([])
  ...
  const fetchReportes = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Filtrar solo reportes del día anterior
        const yesterdayReports = filterYesterdayReports(data.reportes)
        setReportesYesterday(yesterdayReports)
      } else {
  ...
  const getAllSupervisoresWithReportes = () => {
    ...
    const gruposSupervisores = groupReportesBySupervisor(reportesYesterday)
      ...
      <div className="sticky top-0 z-40 bg-white shadow-sm border-b">
        <div className="px-4 py-3">
          ...
          <p className="text-xs sm:text-sm text-gray-600 mt-1">
            Mantenimiento Mecánico - Reportes del {formatDateSpanish(getYesterdayDate())}
          </p>
        </div>
      </div>
      ...
      <div className="px-3 sm:px-4 lg:px-8 py-4 max-w-7xl mx-auto">
        ...
        <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
          <div className="flex items-center justify-between text-sm">
            <div>
              <span className="font-semibold text-blue-900">📅 Mostrando reportes del:</span>
              <span className="ml-2 text-blue-700">{formatDateSpanish(getYesterdayDate())}</span>
            </div>
            <div className="text-xs text-blue-600">
              Total en BD: {reportes.length} | Filtrados: {reportesYesterday.length}
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2 sm:gap-4 mb-4">
          <div className="bg-blue-50 p-3 sm:p-4 rounded-lg text-center">
            <h3 className="text-xs sm:text-sm font-semibold text-blue-900 mb-1">Total Ayer</h3>
            <p className="text-xl sm:text-3xl font-bold text-blue-600">{reportesYesterday.length}</p>
          </div>
          <div className="bg-green-50 p-3 sm:p-4 rounded-lg text-center">
            <h3 className="text-xs sm:text-sm font-semibold text-green-900 mb-1">Terminados</h3>
            <p className="text-xl sm:text-3xl font-bold text-green-600">
              {reportesYesterday.filter(r => r.terminado).length}
            </p>
          </div>
        ...
        {Object.keys(todosSupervisoresConReportes).length > 0 && (
          <div className="space-y-3">
            <h2 className="text-lg sm:text-xl font-semibold text-gray-900 px-1">
              Reportes por Supervisor - {formatDateSpanish(getYesterdayDate())}
            </h2>
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between text-xs sm:text-sm text-gray-600">
                        <span>
                          {datos.reportes.length} reportes ayer • {totalHoras}h
                        </span>
                        ...
                        {(() => {
                          ...
                          return tecnicosSinReportar.length > 0 && (
                            <span className="text-xs bg-orange-100 text-orange-800 px-2 py-1 rounded-full">
                              📧 {tecnicosSinReportar.length} sin reporte ayer
                            </span>
                          )
                        })()}
                      </div>
                    ...
                    <div className={`${typeof window !== 'undefined' && window.innerWidth <= 768 ? (isExpanded ? 'block' : 'hidden') : 'block'}`}>
                        ...
                        <div className="px-3 sm:px-4 pb-3">
                          <div className="text-center py-3 text-gray-500 bg-gray-100 rounded-lg">
                            <p className="text-xs sm:text-sm">
                              📋 No hay reportes del día anterior
                            </p>
                          </div>
                        </div>
                      )}
                      ...
                      <div className="px-3 sm:px-4 pb-3 sm:pb-4">
                        ...
                        >
                          📬 {tieneReportes ? 'Enviar Reporte de Ayer' : 'Enviar Sin Actividad Ayer'}
                        </button>
                      </div>
                    </div>
                  </div>
        ...
        {Object.keys(todosSupervisoresConReportes).length === 0 && !loading && (
          <div className="text-center py-8 text-gray-500">
            <p className="text-sm sm:text-base">No hay reportes del día anterior</p>
            <p className="text-xs text-gray-400 mt-1">
              ({reportes.length} reportes totales en la base de datos)
            </p>
            <button
              onClick={() => {
                fetchReportes()
